...
class TextCNN:
    def __init__(self, filter_sizes, num_filters, num_classes, learning_rate, batch_size, decay_steps, decay_rate,
        ...
        self.b1 = tf.Variable(tf.random_normal([self.num_filters]) / 10)
        self.b2 = tf.Variable(tf.random_normal([self.num_filters]) / 10)
    ...
    def loss(self, l2_lambda=0.0001):  # 0.001
        with tf.name_scope("loss"):
            ...
            # labels_one_hot = tf.reshape(labels_one_hot, [self.batch_size, self.num_classes])
        ...
        for i in range(500):
            ...
            loss, possibility, W_projection_value, _ = sess.run(
                ...
                feed_dict={textRNN.input_x: input_x, textRNN.input_y_multilabel: input_y_multilabel,
                           textRNN.dropout_keep_prob: dropout_keep_prob, textRNN.iter: i, textRNN.tst: not is_training})
            if (i % 100 == 0):
                print(i, "loss:", loss, "-------------------------------------------------------")

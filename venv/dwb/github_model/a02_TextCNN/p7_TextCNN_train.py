...
tf.app.flags.DEFINE_integer("decay_steps", 2000, "how many steps before decay learning rate.")
tf.app.flags.DEFINE_float("decay_rate", 0.8, "Rate of decay for learning rate.")  # 0.8一次衰减多少
tf.app.flags.DEFINE_string("ckpt_dir",
                           "/Users/liyangyang/PycharmProjects/mypy/venv/dwb/github_model/a02_TextCNN/text_cnn_checkpoint/",
...
tf.app.flags.DEFINE_integer("num_filters", 3, "number of filters")
...
def main(_):
    ...
    train, test = data_util.load_data_multilabel(training_data_path, vocabulary_word2index, vocabulary_label2index,
                                                 200)
    trainX, trainY = train
    testX, testY = test
        ...
        for epoch in range(curr_epoch, FLAGS.num_epochs):
            ...
            for start, end in zip(range(0, number_of_training_data, batch_size),
                ...
                curr_loss, lr, curr_acc, _ = sess.run(
                    [textCNN.loss_val, textCNN.learning_rate, textCNN.accuracy, textCNN.train_op], feed_dict)
                loss, counter, acc = loss + curr_loss, counter + 1, acc + curr_acc
                if counter % 2 == 0:
                    print("Epoch %d\tBatch %d\tTrain Loss:%.3f\tLearning rate:%.5f\tTrain Accuracy:%.3f" % (
                        epoch, counter, loss / float(counter), lr, acc / float(counter)))
            ...
            if epoch % FLAGS.validate_every == 0:
                ...
                eval_loss, eval_acc = do_eval(sess, textCNN, testX, testY, iteration, batch_size)
...
# 在验证集上做验证，报告损失、精确度
def do_eval(sess, textCNN, evalX, evalY, iteration, batch_size):
...
def assign_pretrained_word_embedding(sess, vocabulary_index2word, vocab_size, textCNN):
    # word2vecc=word2vec.load('word_embedding.txt') #load vocab-vector fiel.word2vecc['w91874']
    word2vec_dict = word2vec.Word2Vec.load(path + 'word2vec/model')
    # word2vec_dict = {}
